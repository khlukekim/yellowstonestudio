...
void setup(){
  ...
  audioManager.init(useSong, useMic, useBeat);
...
void draw(){
  ...
  for(int i = 0; i<noRings; i++){
    ...
    if(useBeat){
      mod = audioManager.onAnyBeat?5:1;
    }else{
      float level = 0;
      for(int j = 0; j<bandsPerRing; j++){
        level+=audioManager.getLevel(i*bandsPerRing+j, i*bandsPerRing+j+1);
      }
      mod = 10*(log(level)-2);
    }
...
void initRings(){
  if (noRings > 0) {
    ...
    bandsPerRing = audioManager.getSpecSize()/noRings;
    colorMode(HSB, 1);
...
boolean getKey(char k1, int k2) {
  return (javascript == null && key == k1) ||
    (javascript != null && key == k2);
}

void chagneFullScreen() {
  if (javascript == null) {
